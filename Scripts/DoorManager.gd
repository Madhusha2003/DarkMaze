	...
	for x in range(1, maze_width - 1):
		for z in range(1, maze_height - 1):
			if maze_data[x][z] != 1:
				continue
			# Vertical corridor
			if maze_data[x][z-1] == 1 and maze_data[x][z+1] == 1 \
			and maze_data[x-1][z] == 0 and maze_data[x+1][z] == 0:
			...
			# Horizontal corridor
			elif maze_data[x-1][z] == 1 and maze_data[x+1][z] == 1 \
			and maze_data[x][z-1] == 0 and maze_data[x][z+1] == 0:
...
func _is_corridor(maze_data: Array, x: int, z: int) -> bool:
	...
	if maze_data[x-1][z] == 0: open_neighbors += 1
	if maze_data[x+1][z] == 0: open_neighbors += 1
	if maze_data[x][z-1] == 0: open_neighbors += 1
	if maze_data[x][z+1] == 0: open_neighbors += 1
